...
require_once HELPDESK_PATH.'includes/classes/classMysql.php';
require_once HELPDESK_PATH.'includes/classes/classMysqli.php';
require_once HELPDESK_PATH.'includes/classes/classInput.php';
...
('tickets_replies', '10'),
('helpdeskz_version', '".HELPDESK_VERSION."'),
('closeticket_time', '72'),
...
function helpdeskz_saveConfigFile($db_host, $db_name, $db_user, $db_password, $db_prefix, $db_type){
	...
	?>';
	if ( ! file_put_contents(HELPDESK_PATH . 'includes/config.php', $content) )
	{
		return false;
	}else{
		return true;	
...
<p>Welcome to HelpDesk installation process! This will be easy and fun. If you need help, take a look to the ReadMe documentation
(readme.html)</p>
    <p>If you have new ideas to improve the software, feel free to contact us:</p>
<ul>
<li><a href="#">Community</a></li>
    <li><a href="#">Helpdesk ticket</a></li>
</ul>
...
function helpdeskz_checksetup(){
	...
	if ( ! is_writable(HELPDESK_PATH . 'includes/config.php') )
	{
		// -> try to CHMOD it
		if ( function_exists('chmod') )
		{
			@chmod(HELPDESK_PATH . 'includes/config.php', 0666);
		}
    ...
    $attach_dir = HELPDESK_PATH . 'uploads';
    ...
    $attach_dir = HELPDESK_PATH . 'uploads/articles';
	if ( ! file_exists($attach_dir) )
	{
	    @mkdir($attach_dir, 0755);
	}
    ...
    $attach_dir = HELPDESK_PATH . 'uploads/tickets';
	if ( ! file_exists($attach_dir) )
	{
	    @mkdir($attach_dir, 0755);
	}
...
function helpdeskz_database($error_msg =null){
    ...
    <h3>HelpDesk login details</h3>

    <p>Username and password you will use to login into HelpDesk administration.</p>
		<table>
		<tr>
		...
		<tr>
            ...
            <input type="submit" name="btn" value="Install HelpDesk" /></td>
        </tr>
		</table>
    </form>
    <?php
...
if($input->p['license'] == 'agree'){
	if($input->p['settings'] == 'install'){
		...
		if($error_msg != ''){
			...
			helpdeskz_database('Enter the HelpDesk login details.');
		}else{
